...
static const byte kEccPubKeyXRaw[] = {
    0x55, 0xBF, 0xF4, 0x0F, 0x44, 0x50, 0x9A, 0x3D, 0xCE, 0x9B, 0xB7, 0xF0, 0xC5, 0x4D, 0xF5, 0x70,
    0x7B, 0xD4, 0xEC, 0x24, 0x8E, 0x19, 0x80, 0xEC, 0x5A, 0x4C, 0xA2, 0x24, 0x03, 0x62, 0x2C, 0x9B
};
static const byte kEccPubKeyYRaw[] = {
    0xDA, 0xEF, 0xA2, 0x35, 0x12, 0x43, 0x84, 0x76, 0x16, 0xC6, 0x56, 0x95, 0x06, 0xCC, 0x01, 0xA9,
    0xBD, 0xF6, 0x75, 0x1A, 0x42, 0xF7, 0xBD, 0xA9, 0xB2, 0x36, 0x22, 0x5F, 0xC7, 0x5D, 0x7F, 0xB4
};
...
int TPM2_Wrapper_Test(void* userCtx)
{
    ...
    /* RSA Public Key Load */
    rc = wolfTPM2_LoadRsaPublicKey(&dev, &publicKey, kRsaPubKeyRaw, sizeof(kRsaPubKeyRaw), 0);
    if (rc != 0) goto exit;
    ...
    rc = wolfTPM2_LoadEccPublicKey(&dev, &publicKey, TPM_ECC_NIST_P256,
        kEccPubKeyXRaw, sizeof(kEccPubKeyXRaw),
        kEccPubKeyYRaw, sizeof(kEccPubKeyYRaw));
    if (rc != 0) goto exit;
    rc = wolfTPM2_UnloadHandle(&dev, &publicKey.handle);
    if (rc != 0) goto exit;
