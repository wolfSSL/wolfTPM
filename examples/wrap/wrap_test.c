...
static int TPM2_CryptoDevCb(int devId, wc_CryptoInfo* info, void* ctx)
{
    ...
    if (info->algo_type == WC_ALGO_TYPE_PK && info->pk.type == WC_PK_TYPE_RSA) {
        WOLFTPM2_KEY* rsaKey = (WOLFTPM2_KEY*)ctx;
        WOLFTPM2_DEV* dev = rsaKey->handle.dev;

        /* use devId as handle to key for operation */
        switch (info->pk.rsa.type) {
            case RSA_PRIVATE_ENCRYPT:
            case RSA_PUBLIC_ENCRYPT:
                ret = wolfTPM2_RsaEncrypt(dev, rsaKey, TPM_ALG_NULL,
                    info->pk.rsa.in, info->pk.rsa.inLen,
                    info->pk.rsa.out, (int*)info->pk.rsa.outLen);
                break;
            case RSA_PRIVATE_DECRYPT:
            case RSA_PUBLIC_DECRYPT:
                ret = wolfTPM2_RsaDecrypt(dev, rsaKey, TPM_ALG_NULL,
                    info->pk.rsa.in, info->pk.rsa.inLen,
                    info->pk.rsa.out, (int*)info->pk.rsa.outLen);
                break;
        }
...
int TPM2_Wrapper_Test(void* userCtx)
{
    ...
    rc = wc_CryptoDev_RegisterDevice(tpmDevId, TPM2_CryptoDevCb, &rsaKey);
    if (rc != 0) goto exit;
#endif
