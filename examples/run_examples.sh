...
if [ $NO_FILESYSTEM -eq 0 ]; then
    ...
    # extend tests
    ./examples/nvram/store -nvindex=0x01800203 -extend
    RESULT=$?
    [ $RESULT -ne 0 ] && echo -e "nv extend failed! $RESULT" && exit 1
    ./examples/nvram/read -nvindex=0x01800203 -delete
    RESULT=$?
    [ $RESULT -ne 0 ] && echo -e "nv extend read failed! $RESULT" && exit 1

    if [ $WOLFCRYPT_ENABLE -eq 1 ]; then
        ./examples/nvram/store -nvindex=0x01800203 -extend -xor
        RESULT=$?
        [ $RESULT -ne 0 ] && echo -e "nv extend xor failed! $RESULT" && exit 1
        ./examples/nvram/read -nvindex=0x01800203 -xor -delete
        RESULT=$?
        [ $RESULT -ne 0 ] && echo -e "nv extend read xor failed! $RESULT" && exit 1

        if [ $WOLFCRYPT_DEFAULT -eq 0 ]; then
            ./examples/nvram/store -nvindex=0x01800203 -extend -aes
            RESULT=$?
            [ $RESULT -ne 0 ] && echo -e "nv extend aes failed! $RESULT" && exit 1
            ./examples/nvram/read -nvindex=0x01800203 -aes -delete
            RESULT=$?
            [ $RESULT -ne 0 ] && echo -e "nv extend read aes failed! $RESULT" && exit 1
        fi
    fi
fi
