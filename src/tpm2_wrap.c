    ...
    int timeoutTries)
{
    ...
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_Init failed %d: %s\n", rc, wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
#ifdef DEBUG_WOLFTPM
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_SelfTest failed 0x%x: %s\n", rc, TPM2_GetRCString(rc));
    #endif
        return rc;
    }
#ifdef DEBUG_WOLFTPM
    ...
    WOLFTPM2_KEYBLOB* key)
{
    ...
    DEBUG_PRINTF("Get KeyBlob: %d bytes\n", (int)sz);
...
#ifdef WOLFTPM_DEBUG_VERBOSE
    DEBUG_PRINTF("Get KeyBlob public: %d bytes\n", (int)*pubBufferSz);
    TPM2_PrintBin(pubBuffer, *pubBufferSz);

    DEBUG_PRINTF("Get KeyBlob private: %d bytes\n", (int)*privBufferSz);
...
int wolfTPM2_SelfTest(WOLFTPM2_DEV* dev)
{
    ...
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_SelfTest not allowed on Windows TBS (err 0x%x)\n", rc);
    #endif
...
static int wolfTPM2_GetCapabilities_NoDev(WOLFTPM2_CAPS* cap)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_GetCapability manufacture failed 0x%x: %s\n", rc,
            TPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_GetCapability modes failed 0x%x: %s\n", rc,
            TPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (rc == 0) {
        ...
        if (rc_ifx != 0) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("Error getting Infineon vendor capabilities 0x%x: %s\n",
...
int wolfTPM2_GetHandles(TPM_HANDLE handle, TPML_HANDLE* handles)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_GetCapability handles failed 0x%x: %s\n", rc,
            TPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("Handles Cap: Start 0x%x, Count %d\n", handle, handles->count);
    ...
    WOLFTPM2_SESSION* tpmSession)
{
    ...
    if (tpmSession) {
        ...
        XMEMCPY(session->name.name, tpmSession->handle.name.name,
            session->name.size);
...
#ifdef WOLFTPM_DEBUG_VERBOSE
    DEBUG_PRINTF("Encrypt Secret %d: %d bytes\n", rc, data->size);
    ...
    int encDecAlg)
{
    ...
    if (rc < 0) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_GetNonce failed %d: %s\n", rc, wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (authSesIn.tpmKey != TPM_RH_NULL) {
        ...
        if (rc != 0) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("Building encrypted salt failed %d: %s!\n", rc,
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_StartAuthSession failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (keyIn.size > 0) {
        ...
        if (rc != hashDigestSz) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("KDFa ATH Gen Error %d\n", rc);
        #endif
    ...
    const byte* auth, int authSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_CreatePrimary: failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (pkey->creationHash.size > sizeof(pkey->creationHash.buffer)) {
    ...
    WOLFTPM2_HANDLE* parent, const byte* auth, int authSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_ObjectChangeAuth failed %d: %s\n", rc,
                wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_Load key failed %d: %s\n", rc, wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    const byte* auth, int authSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_Create key failed %d: %s\n", rc, wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_Load Key Handle 0x%x\n", (word32)keyBlob->handle.hndl);
#endif
    ...
    const byte* auth, int authSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_CreateLoaded key failed %d: %s\n",
            rc, wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_LoadExternal: 0x%x\n", (word32)loadExtOut.objectHandle);
#endif
    ...
    TPMT_SYM_DEF_OBJECT* sym, TPM2B_DATA* symSeed, int useIv)
{
    ...
    if (digestSz <= 0) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("SensitiveToPrivate: Invalid name algorithm %d\n", nameAlg);
    #endif
        return BAD_FUNC_ARG;
    }
    ...
    if (outerWrap) {
        ...
        if (rc != symKey.size) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("KDFa STORAGE Gen Error %d\n", rc);
        #endif
            return TPM_RC_FAILURE;
        }
        ...
        if (rc != 0) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("SensitiveToPrivate AES error %d!\n", rc);
        #endif
            return rc;
        }
        ...
        if (rc != hmacKey.size) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("KDFa INTEGRITY Gen Error %d\n", rc);
        #endif
            return rc;
        }
        ...
        if (rc != 0) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("SensitiveToPrivate HMAC error %d!\n", rc);
        #endif
            return rc;
        }
    ...
    WOLFTPM2_KEYBLOB* keyBlob, const TPM2B_PUBLIC* pub, TPM2B_SENSITIVE* sens)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("wolfTPM2_ComputeName: failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("wolfTPM2_EncryptSecret: failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("wolfTPM2_SensitiveToPrivate: failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_Import: failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    byte* seed, word32 seedSz)
{
    ...
    if (keyBlob->handle.auth.size > 0) {
        TPM2B_AUTH* auth = &sens.sensitiveArea.authValue;
        auth->size = keyBlob->handle.auth.size;
        if (auth->size > sizeof(auth->buffer)) {
            auth->size = sizeof(auth->buffer);
        }
        XMEMCPY(auth->buffer, keyBlob->handle.auth.buffer, auth->size);
    }
    ...
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("Import RSA name alg size invalid! %d\n", digestSz);
    #endif
        return BUFFER_E;
    }
    if (seed != NULL) {
    ...
    TPMA_OBJECT attributes, byte* seed, word32 seedSz)
{
    ...
    if (digestSz == 0 ||
        digestSz > sizeof(sens.sensitiveArea.seedValue.buffer)) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("Import ECC name alg size invalid! %d\n", digestSz);
    #endif
        return BUFFER_E;
    }
    if (seed != NULL) {
    ...
    const TPM_HANDLE handle)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_ReadPublic failed %d: %s\n", rc, wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    TPM2B_PUBLIC* pub, TPM2B_SENSITIVE* sens, TPMA_OBJECT attributes)
{
    ...
    if (rc == 0) {
        ...
        if (rc == 0 && sens != NULL && isPrivateKey &&
            ...
            rc = BUFFER_E;
        if (rc == 0) {
            TPMS_RSA_PARMS* rsa = &pub->publicArea.parameters.rsaDetail;
            /* Set up public key */
            pub->publicArea.type = TPM_ALG_RSA;
            pub->publicArea.nameAlg = WOLFTPM2_WRAP_DIGEST;
            pub->publicArea.objectAttributes = attributes;
            ...
            if ((attributes & TPMA_OBJECT_fixedParent) ||
                ...
                rsa->symmetric.algorithm = TPM_ALG_NULL;
            }
    ...
    TPM2B_SENSITIVE* sens, TPMA_OBJECT attributes)
{
    ...
    if (rc == 0) {
        ...
        if (rc == 0 && sens != NULL && isPrivateKey &&
            ...
            rc = BUFFER_E;
        if (rc == 0) {
            TPMS_ECC_PARMS* ecc = &pub->publicArea.parameters.eccDetail;
            /* Set up public key */
            pub->publicArea.type = TPM_ALG_ECC;
            pub->publicArea.nameAlg = WOLFTPM2_WRAP_DIGEST;
            pub->publicArea.objectAttributes = attributes;
    ...
    TPMA_OBJECT objectAttributes, byte* seed, word32 seedSz)
{
    ...
    if (rc == 0 && parentKey != NULL) {
        ...
        if (digestSz == 0 ||
            digestSz > sizeof(sens.sensitiveArea.seedValue.buffer)) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("Import %s name alg size invalid! %d\n",
    ...
    WOLFTPM2_KEY* key, TPM_HANDLE persistentHandle)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
        ...
        if (rc == (int)TPM_E_COMMAND_BLOCKED) { /* 0x80280400 */
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("TPM2_EvictControl (storing key to NV) not allowed on "
    ...
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_EvictControl failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    TPMI_ALG_SIG_SCHEME sigAlg, TPMI_ALG_HASH hashAlg)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_Sign failed %d: %s\n", rc, wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    if (key->pub.publicArea.type == TPM_ALG_ECC) {
        ...
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("TPM2_Sign: ECC result buffer too small %d -> %d\n",
                sigOutSz, *sigSz);
        #endif
            return BUFFER_E;
        }
        ...
        if (sigOutSz > *sigSz) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("TPM2_Sign: RSA result buffer too small %d -> %d\n",
                sigOutSz, *sigSz);
        #endif
            return BUFFER_E;
        }
    ...
    const byte* digest, int digestSz, byte* sig, int* sigSz)
{
    ...
    pub = &key->pub.publicArea;
    if (pub->type == TPM_ALG_ECC) {
        /* Keys that are created with sign and decrypt require scheme to
         * be NULL, but we must supply ECDSA and Hash Algorithm for signing */
        sigAlg = pub->parameters.eccDetail.scheme.scheme;
        hashAlg = pub->parameters.eccDetail.scheme.details.any.hashAlg;
        ...
        sigAlg = pub->parameters.rsaDetail.scheme.scheme;
    ...
    TPMT_TK_VERIFIED* checkTicket)
{
    ...
    TPMU_SIGNATURE* signature = &verifySigIn.signature.signature;
    ...
    if (key->pub.publicArea.type == TPM_ALG_ECC) {
        ...
        if (curveSize > (int)sizeof(signature->ecdsa.signatureR.buffer)) {
            return BAD_FUNC_ARG;
        }
        ...
        if (sigSz > (int)sizeof(signature->rsassa.sig.buffer)){
            return BAD_FUNC_ARG;
        }
    }
    else {
    ...
    if (key->pub.publicArea.type == TPM_ALG_ECC) {
        ...
        signature->ecdsa.signatureR.size = curveSize;
        XMEMCPY(signature->ecdsa.signatureR.buffer, sig, curveSize);
        signature->ecdsa.signatureS.size = curveSize;
        XMEMCPY(signature->ecdsa.signatureS.buffer, sig + curveSize, curveSize);
    }
    else if (key->pub.publicArea.type == TPM_ALG_RSA) {
        signature->rsassa.sig.size = sigSz;
        XMEMCPY(signature->rsassa.sig.buffer, sig, sigSz);
    }
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_VerifySignature failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
    }
    else {
    ...
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_VerifySignature: Tag %d\n",
    ...
    TPM2B_ECC_POINT* pubPoint, byte* out, int* outSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_ECDH_KeyGen failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    const TPM2B_ECC_POINT* pubPoint, byte* out, int* outSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_ECDH_ZGen failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_ECDH_ZGen: zPt %d\n", ecdhZOut.outPoint.size);
#endif
...
int wolfTPM2_ECDHEGenKey(WOLFTPM2_DEV* dev, WOLFTPM2_KEY* ecdhKey, int curve_id)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_EC_Ephemeral failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    wolfTPM2_CopyEccParam(&ecdhKey->pub.publicArea.unique.ecc.x,
        &out.Q.point.x);
    wolfTPM2_CopyEccParam(&ecdhKey->pub.publicArea.unique.ecc.y,
        &out.Q.point.y);
    ...
    byte* out, int* outSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_ZGen_2Phase failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_ZGen_2Phase: zPt %d\n", outZGen2Ph.outZ2.size);
#endif
    ...
    TPM_ALG_ID padScheme, const byte* msg, int msgSz, byte* out, int* outSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_RSA_Encrypt failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_RSA_Encrypt: %d\n", rsaEncOut.outData.size);
#endif
    ...
    TPM_ALG_ID padScheme, const byte* in, int inSz, byte* msg, int* msgSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_RSA_Decrypt failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_RSA_Decrypt: %d\n", rsaDecOut.message.size);
#endif
    ...
    int* pDigestLen)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_PCR_Read failed %d: %s\n", rc, wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    const byte* digest, int digestLen)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_PCR_Extend failed 0x%x: %s\n", rc, TPM2_GetRCString(rc));
    #endif
    }

#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_PCR_Extend: Index %d, Digest Sz %d\n", pcrIndex, digestLen);
#endif
...
int wolfTPM2_UnloadHandle(WOLFTPM2_DEV* dev, WOLFTPM2_HANDLE* handle)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_FlushContext failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_FlushContext: Closed handle 0x%x\n", (word32)handle->hndl);
#endif
    ...
    const byte* auth, int authSz, const byte* authPolicy, int authPolicySz)
{
    ...
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_NV_DefineSpace failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    word32 nvIndex, byte* dataBuf, word32 dataSz, word32 offset, int extend)
{
    ...
    while (dataSz > 0) {
        ...
        if (rc != TPM_RC_SUCCESS) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("wolfTPM2_NVWriteData: Setting NV index name failed\n");
        #endif
            rc = TPM_RC_FAILURE;
            break;
        }
    ...
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("wolfTPM2_NVWriteData: Auth 0x%x, Idx 0x%x, Offset %d, Size %d, "
            "Extend %d\n",
...
#ifdef DEBUG_WOLFTPM
    if (rc != TPM_RC_SUCCESS) {
        DEBUG_PRINTF("wolfTPM2_NVWriteData failed %d: %s\n",
            rc, wolfTPM2_GetRCString(rc));
    }
#endif
...
#ifdef DEBUG_WOLFTPM
    if (rc != TPM_RC_SUCCESS) {
        DEBUG_PRINTF("TPM2_NV_Read failed %d: %s\n", rc, wolfTPM2_GetRCString(rc));
    }
#endif
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("NV public read certificate 0x%x (%u bytes)\n",
        handle, *len);
    ...
    TPMS_NV_PUBLIC* nvPublic)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_NV_ReadPublic failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_NV_ReadPublic: Sz %d, Idx 0x%x, nameAlg %d, Attr 0x%x, "
...
int wolfTPM2_NVIncrement(WOLFTPM2_DEV* dev, WOLFTPM2_NV* nv)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_NV_Increment failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    word32 nvIndex)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_NV_UndefineSpace failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
int wolfTPM2_GetRandom(WOLFTPM2_DEV* dev, byte* buf, word32 len)
{
    ...
    while (pos < len) {
        ...
        if (rc != TPM_RC_SUCCESS) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("TPM2_GetRandom failed 0x%x: %s\n", rc,
                TPM2_GetRCString(rc));
        #endif
            break;
        }
...
int wolfTPM2_Clear(WOLFTPM2_DEV* dev)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_Clear failed %d: %s\n", rc,
            wolfTPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM2_Clear Auth 0x%x\n", (word32)in.authHandle);
#endif
    ...
    TPMI_ALG_HASH hashAlg, const byte* usageAuth, word32 usageAuthSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_HashSequenceStart failed 0x%x: %s\n", rc,
            TPM2_GetRCString(rc));
    #endif
        return rc;
    }
    ...
    const byte* data, word32 dataSz)
{
    ...
    while (pos < dataSz) {
        ...
        if (rc != TPM_RC_SUCCESS) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("TPM2_SequenceUpdate failed 0x%x: %s\n", rc,
                TPM2_GetRCString(rc));
        #endif
            return rc;
        }
    ...
    byte* digest, word32* digestSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_SequenceComplete failed 0x%x: %s: Handle 0x%x\n", rc,
    ...
    const TPMT_SENSITIVE* sensitive, TPM2B_DIGEST* unique)
{
...
#ifdef WOLFTPM_USE_SYMMETRIC
    uniqueSz = TPM2_GetHashDigestSize(hashAlg);
    ...
    if (rc == 0) {
        ...
        if (rc == 0) {
            /* sensitive value */
            rc = wolfTPM2_HashUpdate(dev, &hash,
                sensitive->sensitive.any.buffer, sensitive->sensitive.any.size);
        }
        if (rc == 0) {
    ...
    const byte* keyBuf, word32 keySz)
{
    ...
    if (key->handle.auth.size > 0) {
        TPM2B_AUTH* auth = &loadExtIn.inPrivate.sensitiveArea.authValue;
        auth->size = key->handle.auth.size;
        if (auth->size > sizeof(auth->buffer))
            auth->size = sizeof(auth->buffer);
        XMEMCPY(auth->buffer, key->handle.auth.buffer, auth->size);
    }
    ...
    int isDecrypt)
{
    ...
    if (inOutSz > sizeof(encDecIn.inData.buffer)) {
        return BUFFER_E;
    }
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_EncryptDecrypt2 failed 0x%x: %s\n", rc,
            TPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
int wolfTPM2_SetCommand(WOLFTPM2_DEV* dev, TPM_CC commandCode, int enableFlag)
{
    ...
    if (TPM2_GetVendorID() == TPM_VENDOR_STM) {
        ...
        if (rc != TPM_RC_SUCCESS) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("TPM2_SetCommandSet failed 0x%x: %s\n", rc,
                TPM2_GetRCString(rc));
        #endif
        }
    }
    ...
    word32 keySz, const byte* usageAuth, word32 usageAuthSz)
{
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_HMAC_Start failed 0x%x: %s\n", rc,
            TPM2_GetRCString(rc));
    #endif
        return rc;
    }
...
int wolfTPM2_Reset(WOLFTPM2_DEV* dev, int doShutdown, int doStartup)
{
    ...
    if (doShutdown == 1) {
        ...
        if (rc != TPM_RC_SUCCESS) {
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("TPM2_Shutdown failed 0x%x: %s\n",
                rc, TPM2_GetRCString(rc));
        #endif
        }
    }
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("wolfTPM2_Reset complete\n");
#endif
    ...
    TPMI_RH_HIERARCHY_AUTH authHandle)
{
    ...
    if (rc == 0) {
        ...
        in.newAuth.size = (UINT16)TPM2_GetHashDigestSize(WOLFTPM2_WRAP_DIGEST);
    ...
    if (rc == 0) {
        DEBUG_PRINTF("%s auth set to %d bytes of random\n", desc, in.newAuth.size);
        #ifdef WOLFTPM_DEBUG_VERBOSE
            DEBUG_PRINTF("\tAuth Sz %d\n", in.newAuth.size);
            TPM2_PrintBin(in.newAuth.buffer, in.newAuth.size);
        #endif
    } else {
        DEBUG_PRINTF("Error %d setting %s auth! %s\n",
    ...
    switch (publicTemplate->type) {
        case TPM_ALG_RSA:
            ...
            if (uniqueSz > (int)sizeof(publicTemplate->unique.rsa.buffer)) {
                uniqueSz = (int)sizeof(publicTemplate->unique.rsa.buffer); /* truncate */
            }
            if (unique == NULL) {
    ...
    word32 pcrArraySz, const byte* sealData, int sealSize)
{
    ...
    XMEMCPY(createIn.inSensitive.sensitive.data.buffer, sealData, sealSize);
...
int wolfTPM2_GetTime(WOLFTPM2_KEY* aikKey, GetTime_Out* getTimeOut)
{
    ...
    if (aikKey != NULL) {
        ...
        getTimeCmd.inScheme.details.any.hashAlg =
            scheme->details.anySig.hashAlg;
    }
    else {
    ...
    if (rc != TPM_RC_SUCCESS) {
    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("TPM2_GetTime failed 0x%x: %s\n", rc, wolfTPM2_GetRCString(rc));
    #endif
    }
...
static void wolfTPM2_CopyPubT(TPMT_PUBLIC* out, const TPMT_PUBLIC* in)
{
    ...
    if (out->authPolicy.size > 0) {
        if (out->authPolicy.size > (UINT16)sizeof(out->authPolicy.buffer)) {
            out->authPolicy.size =
                (UINT16)sizeof(out->authPolicy.buffer); /* truncate */
        }
        XMEMCPY(out->authPolicy.buffer, in->authPolicy.buffer,
            out->authPolicy.size);
    }
    ...
    case TPM_ALG_KEYEDHASH:
        ...
        if (out->unique.keyedHash.size >
            ...
            out->unique.keyedHash.size =
                (UINT16)sizeof(out->unique.keyedHash.buffer); /* truncate */
        }
        XMEMCPY(out->unique.keyedHash.buffer, in->unique.keyedHash.buffer,
            out->unique.keyedHash.size);
        break;
    case TPM_ALG_SYMCIPHER:
        ...
        out->unique.sym.size =
            in->unique.sym.size;
        if (out->unique.sym.size > (UINT16)sizeof(out->unique.sym.buffer)) {
            out->unique.sym.size =
                (UINT16)sizeof(out->unique.sym.buffer); /* truncate */
        }
        XMEMCPY(out->unique.sym.buffer, in->unique.sym.buffer,
                out->unique.sym.size);
        break;
    case TPM_ALG_RSA:
        ...
        out->unique.rsa.size =
            in->unique.rsa.size;
        if (out->unique.rsa.size > (UINT16)sizeof(out->unique.rsa.buffer)) {
            out->unique.rsa.size =
                (UINT16)sizeof(out->unique.rsa.buffer); /* truncate */
        }
        XMEMCPY(out->unique.rsa.buffer, in->unique.rsa.buffer,
            out->unique.rsa.size);
        break;
    case TPM_ALG_ECC:
    ...
    const byte* auth, int authSz)
{
    ...
    if (auth != NULL && authSz >= 0) {
        if (authSz > (int)sizeof(tpmSession->handle.auth.buffer)) {
            authSz = (int)sizeof(tpmSession->handle.auth.buffer); /* truncate */
        }
    ...
    const byte* policyRef, word32 policyRefSz)
{
    ...
    if (pcrDigestSz > sizeof(policyAuthIn.approvedPolicy.buffer)) {
        return BUFFER_E;
    }
    ...
    /* policyRef (nonce) */
    if (policyRefSz > sizeof(policyAuthIn.policyRef.buffer)) {
        return BUFFER_E;
    }
...
#ifdef DEBUG_WOLFTPM
    if (rc != TPM_RC_SUCCESS) {
        DEBUG_PRINTF("PolicyAuthorize failed %d: %s\n", rc, wolfTPM2_GetRCString(rc));
    }
#endif
    return rc;
}
    ...
    byte* pcrArray, word32 pcrArraySz, byte* pcrDigest, word32* pcrDigestSz)
{
    ...
    *pcrDigestSz = rc; /* set actual size */
    ...
    if (rc != 0) {
        ...
        DEBUG_PRINTF("wolfTPM2_PCRGetDigest: %d\n", *pcrDigestSz);
...
#ifdef DEBUG_WOLFTPM
    if (rc != 0) {
        DEBUG_PRINTF("wolfTPM2_PolicyHash failed %d: %s\n",
            rc, wolfTPM2_GetRCString(rc));
    }
    #ifdef WOLFTPM_DEBUG_VERBOSE
    else {
        DEBUG_PRINTF("wolfTPM2_PolicyHash: %d\n", *digestSz);
    ...
    const byte* pcrDigest, word32 pcrDigestSz, byte* digest, word32* digestSz)
{
    ...
    if (rc != 0) {
        ...
        DEBUG_PRINTF("wolfTPM2_PolicyPCRMake: %d\n", *digestSz);
        TPM2_PrintBin(digest, *digestSz);
    }
    #endif
#endif
    ...
    const byte* policyRef, word32 policyRefSz)
{
    ...
    if (rc != 0) {
        ...
        DEBUG_PRINTF("wolfTPM2_PolicyAuthorizeMake: %d\n", *digestSz);
        TPM2_PrintBin(digest, *digestSz);
    }
    #endif
#endif
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("TPM Serial Number (%d bytes)\n", (int)sizeof(serialNum));
...
#ifdef DEBUG_WOLFTPM
    DEBUG_PRINTF("Handle 0x%x, Auth %d\n", handle->hndl, handle->auth.size);
    TPM2_PrintBin(handle->auth.buffer, handle->auth.size);
#endif
    ...
    wolfTPM2FwDataCb cb, void* cb_ctx)
{
    ...
    for (offset = 0; ; offset += chunk_sz) {
        ...
        #ifdef DEBUG_WOLFTPM
            DEBUG_PRINTF("Firmware data callback error! %d\n", rc);
        #endif
            break;
        }

    #ifdef DEBUG_WOLFTPM
        DEBUG_PRINTF("Firmware data chunk offset %u\n", offset);
    #endif
